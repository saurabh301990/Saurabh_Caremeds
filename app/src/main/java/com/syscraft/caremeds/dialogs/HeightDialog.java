...
public class HeightDialog extends DialogFragment {
    ...
    private HeightListener callback;
    View dialoglayout;
    EditText edtNewHeight;
    int position;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        try {
            callback = (HeightListener)getActivity();
    ...
    public Dialog onCreateDialog(Bundle savedInstanceState) {
        ...
        dialoglayout = inflater.inflate(R.layout.update_height, null);
        builder1.setView(dialoglayout);
        //builder1.setInverseBackgroundForced(true);
        Bundle args = getArguments();
        position = args.getInt("position");
        //  builder1.show();
        edtNewHeight = (EditText)dialoglayout.findViewById(R.id.edtNewHeight);
        builder1.setPositiveButton(
                "Confirm",
                new DialogInterface.OnClickListener() {
                    public void onClick(DialogInterface dialog, int id) {
                        if (edtNewHeight.getText().toString().length() > 0) {
                            //callback.HeightUpdate(edtNewHeight.getText().toString(), position);
                            int a=Integer.parseInt(edtNewHeight.getText().toString());
                            callback.HeightUpdate(a, position);
                        }
                        dialog.dismiss();
                    }
                });
