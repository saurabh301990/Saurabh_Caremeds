...
public class PatientDetail extends BaseActivity implements View.OnClickListener, AdapterView.OnItemSelectedListener,
    ...
    TextView txtyesterday, txttoday, txtommrow;
    private String prescribed_time_slots_for_patient_list_array;
...
//        db = new CareMedsDatabase(this);
        ...
        jsonParser = new JSONParser(PatientDetail.this);
...
//      progDialog.setMessage("Please wait");
        ...
        if (i.getStringExtra("name") != null && i.getStringExtra("dob") != null && i.getStringExtra("gp") != null
            ...
            prescribed_time_slots_for_patient_list_array = i.getStringExtra("prescribed_time_slots_for_patient_list_array");
        ...
        txtommrow = (TextView) findViewById(R.id.txtommrow);
...
//        BsList = new ArrayList<MeasureModel>();
        ...
        txtommrow.setOnClickListener(this);
...
//                        bld.putString("fullNmae", fullNmaeMar);
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        if (is_anything_remains_to_sync) {
            ...
            if (isNetworkAvailable(PatientDetail.this)) {
    ...
    private void getCheckInData() {
        ...
        if ("This Cycle".equals(cycle)) {
            imc_met = "this_cycle";
            Log.e("NetworkAvailablity", "NetworkAvailablity------->" + NetworkAvailablity.chkStatus(PatientDetail.this));
...
//                App_Constants.showAlertDialog("Alert", "Please check network connection!", PatientDetail.this, false);
    ...
    private void getAdminData(String cycle) {
...
//        } else {
        ...
        if (!patientIs_absent.equalsIgnoreCase("true")) {
            ...
            admin_prescription_modelArrayList = mydbobj.get_admin_tab_prescriptions_FromDB(patientId, cycle);
            ...
            if (admin_prescription_modelArrayList.size() > 0) {
                try {
//                    JSONArray array2 = new JSONArray(admin_prescription_modelArrayList.get(0).getPrescribed_time_slots_array());
                    JSONArray array2 = new JSONArray(prescribed_time_slots_for_patient_list_array);
...
//        }
    ...
    private void getyesterdaydata(String cycle) {
...
//        _global_warfarin_dose = about_List.get(0).get_global_warfarin_dose();
        ...
        _global_inr_reading = mydbobj.get_inr_reading(patientId);
        _global_inr_date = mydbobj.get_inr_date(patientId);
        _global_warfarin_dose = mydbobj.get_warfarin_dose(patientId);

        Log.e("_global_inr_reading", "_global_inr_reading--->" + _global_inr_reading);
        Log.e("_global_inr_date", "_global_inr_date--->" + _global_inr_date);
        Log.e("_global_warfarin_dose", "_global_warfarin_dose--->" + _global_warfarin_dose);
...
//        }
    ...
    private void getMeds() {
        ...
        newmedsAdapter = new NewMedsAdapter(PatientDetail.this, newmedsList, audit_enabled);
...
//        } else {
        ...
        new_return_Adpater = new New_Return_Adpater(PatientDetail.this, new_return_List);
...
//                    getAdminData();
                ...
                } else if (txtyesterday.isSelected()) {
                    Enable_Admin_tab();
...
//                    } else {
                    ...
                    if (require_witness_on_checkin.equalsIgnoreCase("true")) {
                        Toast.makeText(PatientDetail.this, "You are in offline mode, secondary witnesses are not\n" +
                    ...
                    if (txttoday.isSelected()) {
                            ...
                            for (int i = 0; i < new_admin_modelArrayList.size(); i++) {
                                ...
                                if (i == (new_admin_modelArrayList.size() - 1)) {
                                    if (txttoday.isSelected()) {
                                        ...
                                        getAdminData("last_cycle");
                                    } else if (txtommrow.isSelected()) {
                                        getAdminData("next_cycle");
                                    }
                                }
                            }
                        ...
                        if (_IS_SECONDRY_USER_AUTH_REQUIRED) {
                            ...
                            for (int i = 0; i < new_admin_modelArrayList.size(); i++) {
                                ...
                                Log.e("No witness Loop for---->", "" + i + " new_admin_modelArrayList---->" + new_admin_modelArrayList.size());
                                if (nd1.is_edited()) {
                                    create_yesterday_json(nd1);
                                    create_offline_admin(nd1);
                                }
                                if (i == (new_admin_modelArrayList.size() - 1)) {
                                    if (txtyesterday.isSelected()) {
                                        getyesterdaydata("this_cycle");
                                    } else if (txtyesterday.isSelected()) {
                                        getyesterdaydata("last_cycle");
                                    } else if (txtommrow.isSelected()) {
                                        getyesterdaydata("next_cycle");
                                    }
                                }
                            }
                        ...
                        for (int i = 0; i < new_homely_List.size(); i++) {
                            ...
                            if (h1.is_edited()) {
                                Log.e("is_edited id", "*********---->" + h1.getId());
                                create_homely_yesterday_json(h1.getId(), h1.getName(), h1.getDose_to_update(), h1.getPatient_id());
                                create_offline_homely(h1.getId(), h1.getDose_to_update());
                            }
                            if (i == (new_homely_List.size() - 1)) {
                                getHomelyData();
...
//                    }
            ...
            case R.id.btnCancel:
                ...
                if (tvcheck.isSelected()) {
                    ...
                    showTwoBtnAlertDialog("Clear selections?", "Are you Sure?", PatientDetail.this);
                } else if (txtAdmin.isSelected()) {
//                    getAdminData();
                } else if (txtHome.isSelected()) {
                    getHomelyData();
...
//            result = jsonParser.postdata_method(weburl, json.toString());
    ...
    private void create_today_json(NEW_ADMIN_MODEL nd1) {
        ...
        Homely_Today_Mars_Mar_Bean_Phase2 hm = new Homely_Today_Mars_Mar_Bean_Phase2
                ("", datetime, nd1.getDose(), nd1.getReason(), nd1.getIs_waste(), nd1.getOutcome(),
                        nd1.getQuantity_wasted(), nd1.getSlot_time(), datetime, userid, username, userfullname,
                        "", "0", "", nd1.getPrecription_ID(), nd1.getPatient_ID());
    ...
    private void create_yesterday_json(NEW_ADMIN_MODEL nd1) {
        ...
        homely_Today_Mars_Mar_objects_list.add(hm);
        mydbobj.insert_yesterday_marsData(homely_Today_Mars_Mar_objects_list, "offline");
        int updatededid = mydbobj.update_Last_admin(last_admin_json.toString(), nd1.getPrecription_ID());
        Log.e("update_Last_admin", "updatededid------>" + updatededid);
...
//        homely_Today_Mars_Mar_objects_list.add(hm);
    ...
    private void create_homely_yesterday_json(String hid, String hname, String dose_to_update, String patient_id) {
        Log.e("create_homely_yesterday_json", "create_homely_yesterday_json--->");
        String userid = PreferenceConnector.readString(getApplicationContext(), PreferenceConnector.USERID, "");
        String username = PreferenceConnector.readString(getApplicationContext(), PreferenceConnector.USERNAME, "");
        String userfullname = PreferenceConnector.readString(getApplicationContext(), PreferenceConnector.FULLNAME, "");
        ...
        try {
            newdate = date_utility.Patient_detail_getDate(newdatetime);
            Log.e("newdate", "create_homely_yesterday_json newdate--->" + newdate);
//                json1.put("slot_time", newdate);
...
//                Log.e("Loop for" + i, "getPrecription_ID---->" + nd1.getPrecription_ID());
    ...
    private void Enable_Notes_Tab() {
        ...
        txtyesterday.setVisibility(View.GONE);
        txtommrow.setVisibility(View.GONE);
    ...
    private void Enable_Admin_tab() {
        ...
        txtommrow.setVisibility(View.VISIBLE);
        txttoday.setVisibility(View.VISIBLE);

    }


    private void Enable_Today_TAB() {
        Log.e("Enable_Today_TAB", "--->Enable_Today_TAB");
        txtyesterday.setSelected(false);
        txtommrow.setSelected(false);
        txttoday.setSelected(true);
        getAdminData("this_cycle");
    }

    private void Enable_Tommorow_TAB() {
        txtyesterday.setSelected(false);
        txtommrow.setSelected(true);
        txttoday.setSelected(false);
        getAdminData("this_cycle");
    }

    private void Enable_Yesterday_TAB() {
        txtyesterday.setSelected(true);
        txtommrow.setSelected(false);
        txttoday.setSelected(false);
        getyesterdaydata("this_cycle");
    }
    ...
    private void open_witness_dialog(String from) {
        FragmentManager fragmentManager = ((FragmentActivity) PatientDetail.this).getSupportFragmentManager();
    ...
    public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
        ...
        if (tvcheck.isSelected()) {
            ...
            if (spCycle.getSelectedItem().toString().equals("This Cycle")) {
                ...
                check_for_audit();
                Log.e("NetworkAvailablity", "NetworkAvailablity---1---->" + NetworkAvailablity.chkStatus(PatientDetail.this));
...
//                new getMedsData().execute(imc_met);
                ...
                spnPosition1 = position;
                check_for_audit();
                Log.e("NetworkAvailablity", "NetworkAvailablity---2---->" + NetworkAvailablity.chkStatus(PatientDetail.this));
...
//                }
//                mydbobj.addAllFeed(PatientDetail.this, admin_prescription_modelArrayList);
////                getAdmin();
//            } catch (Exception e) {
//                e.printStackTrace();
//            }
            ...
            if (new_homely_List.size() > 0) {
                ...
                for (int i = 0; i < new_homely_List.size(); i++) {
                    if (new_homely_List.get(i).getHomely_Today_Mars_Mar_objects_list().size() > 0) {
                        Homely_Model_Phase2 homely_remedy_model5 = new_homely_List.get(i);
...
//                    ArrayList<TIME_SLOT_DOSES> Tsd_objects_list = admin_prescription_modelArrayList.get(x).getTIME_SLOT_DOSES_objects_list();
                    ...
                    String TIME_SLOT_DOSES_array = admin_prescription_modelArrayList.get(x).getPrescribed_time_slots_array();

                    String p_id = "" + admin_prescription_modelArrayList.get(x).getId();

                    ArrayList<PRESCRIBED_TIME_SLOT_phase2> Tsd_objects_list = gettimeslotlist(TIME_SLOT_DOSES_array, p_id);

                    for (int j = 0; j < Tsd_objects_list.size(); j++) {
                        PRESCRIBED_TIME_SLOT_phase2 timeSlotDoses = Tsd_objects_list.get(j);

                        if (timedosenamelist.get(loop).equals(timeSlotDoses.getShow_as())) {

                            PRESCRIPTION_Model_Phase2 adminPrescriptionModel = admin_prescription_modelArrayList.get(x);

                            if (adminPrescriptionModel.getPrescription_type().equals("Irregular")) {
                                Log.e("SSSSSSSSSSSSS  ", "getPrescription_type----> Irregular");

                                boolean b = check_if_current_date_is_in_Irregular_array(adminPrescriptionModel);


                                if (b == false) {
                                    if (mydbobj.getYesterdayMar(String.valueOf(adminPrescriptionModel.getId()), patientId, timeSlotDoses.getSlot_time()) == false) {

                                        if (timeSlotDoses.getSlot_time().length() > 0) {
                                            Log.e("YESTERDAYCHECK", "FALSE1");
                                            if (!headerlist.contains(timeSlotDoses.getShow_as())) {
                                                NEW_ADMIN_MODEL nam11 = new NEW_ADMIN_MODEL(true, timeSlotDoses.getShow_as(), timeSlotDoses.getColor(), "");
                                                new_admin_modelArrayList.add(nam11);
                                                headerlist.add(timeSlotDoses.getShow_as());
                                            }

                                            NEW_ADMIN_MODEL nam11 = new NEW_ADMIN_MODEL(false, "", timeSlotDoses.getShow_as(), timeSlotDoses.getSlot_time(), null, adminPrescriptionModel, false, timeSlotDoses.getColor());
                                            new_admin_modelArrayList.add(nam11);
                            ...
                            } else if (mydbobj.getYesterdayMar(String.valueOf(adminPrescriptionModel.getId()), patientId, timeSlotDoses.getSlot_time()) == false) {
    ...
    private boolean check_if_current_date_is_in_Irregular_array(PRESCRIPTION_Model_Phase2 adminPrescriptionModel) {
        ...
        String abc = adminPrescriptionModel.getIrregular_grid_array();
    ...
    private void getAdmin(String cycle) {
//        adminList = new ArrayList<AdminInterFace>();
        new_admin_modelArrayList = new ArrayList<NEW_ADMIN_MODEL>();
        headerlist = new ArrayList<String>();
...
//                    ArrayList<TIME_SLOT_DOSES> Tsd_objects_list = admin_prescription_modelArrayList.get(x).getTIME_SLOT_DOSES_objects_list();
                    ...
                    for (int j = 0; j < Tsd_objects_list.size(); j++) {
                        ...
                        if (timedosenamelist.get(loop).equals(timeSlotDoses.getShow_as())) {
                            ...
                            PRESCRIPTION_Model_Phase2 adminPrescriptionModel = admin_prescription_modelArrayList.get(x);
                            String currentdate = Date_utility.getCurrentdate();
                            boolean checkdate = checkdate(admin_prescription_modelArrayList.get(x).getStart_date(), admin_prescription_modelArrayList.get(x).getEnd_date(), currentdate);
                            Log.e("Data", "Check Date :------------->>>>>>>>" + checkdate);
                            if (adminPrescriptionModel.getPrescription_type().equals("Irregular")) {
                                Log.e("SSSSSSSSSSSSS  ", "getPrescription_type----> Irregular");
                                ...
                                if (timeSlotDoses.getDose().length() > 0) {
    ...
    private ArrayList<PRESCRIBED_TIME_SLOT_phase2> gettimeslotlist(String TIME_SLOT_DOSES_array, String p_id) {
        ...
        try {
            ...
            for (int j = 0; j < array2.length(); j++) {
                ...
                String time_slot_doses_show_as = obj6.optString("show_as");
                String time_slot_doses_color = obj6.optString("color");
                String dose = obj6.optString("dose");
                String updated_at = obj6.optString("updated_at");

                PRESCRIBED_TIME_SLOT_phase2 tsd_obj = new PRESCRIBED_TIME_SLOT_phase2
                        (time_slot_doses_id, time_slot_doses_slot_time, time_slot_doses_show_as,
                                time_slot_doses_color, dose, updated_at, p_id);
...
//                    new_return_Adpater = new New_Return_Adpater(PatientDetail.this, new_return_List);
//                    mylist.setAdapter(new_return_Adpater);
//                    mydbobj.addAllFeed(PatientDetail.this, new_return_List);
//                }
//            } catch (Exception e) {
//                e.printStackTrace();
//            }
...
//
//                if (tvmeds.isSelected()) {
//                    mydbobj.addAllFeed(PatientDetail.this, newmedsList);
//                    newmedsAdapter = new NewMedsAdapter(PatientDetail.this, newmedsList, audit_enabled);
//                    mylist.setAdapter(newmedsAdapter);
//
//                }
...
//            progDialog.dismiss();
    ...
    private void open_alert(final String from_val) {
        ...
        AlertDialog.Builder alertDialogBuilder = new AlertDialog.Builder(PatientDetail.this);
        // set title
...
//                                        getAdminData();
                    ...
                    Enable_save_and_cancel_button();
                    App_Constants.showAlertDialog("Authentication failed", "The credentials you entered could not be authenticated.", PatientDetail.this, false);
                }
    ...
    private void blank_alert(String myMessage) {
        AlertDialog.Builder alertDialogBuilder2 = new AlertDialog.Builder(PatientDetail.this);
        // set title
...
//            return builder1.create();
    ...
    private boolean check_is_anything_remains_to_sync() {
        ...
        String caretaker_id = PreferenceConnector.readString(PatientDetail.this, PreferenceConnector.LAST_LOGIN_CARETAKER_ID, "");
...
*/
    public boolean checkdate(String startdate, String enddate, String currentdate) {
        boolean result_new = false;
        SimpleDateFormat formatter = new SimpleDateFormat("dd-MM-yy");
        try {
            Date min = formatter.parse(startdate);
            Date max = formatter.parse(enddate);
            long current_lng = Date.parse(currentdate);
            Date current = new Date(current_lng);

            boolean result = isDateInBetweenIncludingEndPoints(min, max, current);
            if (result == true) {
                result_new = true;
                return result_new;
            }
